...
public class MyScanner {
    ...
    private Boolean isIdentifierFA(String token) throws FileNotFoundException {
        FiniteAutomata FA_identifier = new FiniteAutomata("D:/Facultate/Semestrul-5/Formal-Languages-and-Compiler-Design/Labs/Lab2/SymbolTableImplementation/fa/identifier");
    ...
    private Boolean isConstant(String token) throws FileNotFoundException {
        ...
        FiniteAutomata FA_int_constant = new FiniteAutomata("D:/Facultate/Semestrul-5/Formal-Languages-and-Compiler-Design/Labs/Lab2/SymbolTableImplementation/fa/int_constant");

        FA_int_constant.readFromFile();
        if(!FA_int_constant.isDFA()){
            return false;
        }
...
//                || token.equals("false")
//                || (m!=null && m.matches());
        return FA_int_constant.isAccepted(token) // numberconst
                || token.matches("'[a-zA-Z0-9._-]'") // charconst
                || token.equals("true")
                || token.equals("false")
                || (m!=null && m.matches());
    ...
    public void classifyTokens() throws IOException {
        ...
        for (Pair<String, Integer> pair: this.detectedTokens) {
            if (isReservedOperatorSeparator(pair.getKey()) || isReservedOperatorSeparator(pair.getKey().toLowerCase())) {
                pw.printf("%-20s %d\n", pair.getKey(), -1);
            } else if (isIdentifierFA(pair.getKey()) || isConstant(pair.getKey())) {
